...
int main() {
    ...
    for(int i=1;i<sectionTab[1];i++)vertTab[0].emplace_back(i,rand()%17+5);
    for(int j=1;j<sectionN;j++)for(int i=sectionTab[j-1];i<sectionTab[j];i++){
        if(i==0)continue;
        int connectionN;
        set<int> uniqueConnections;
        connectionN=(sectionTab[j+1]-sectionTab[j])/5+rand()%2+1;
        while(uniqueConnections.size()<connectionN-1)uniqueConnections.insert(rand()%(sectionTab[j+1]-sectionTab[j]+1)+sectionTab[j]);
        for(auto it=uniqueConnections.begin();it!=uniqueConnections.end();it++)vertTab[i].emplace_back(*it,rand()%17+5);
    }
    for(int i=sectionTab[sectionN-1];i<vertN+1;i++)vertTab[i].emplace_back(vertN+1,rand()%17+5);
    for(int i=0;i<vertN+1;i++)finalConnectionN+=vertTab[i].size();
    ofstream file("daneZwagami.txt");
    file<<vertN+2<<" "<<finalConnectionN;
    for(int i=0;i<vertN+1;i++)for(int j=0;j<vertTab[i].size();j++)file<<endl<<i<<" "<<vertTab[i][j].first<<" "<<vertTab[i][j].second;
