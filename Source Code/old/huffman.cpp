...
int main() {
    ...
    huff.writeToFile("output.huff", data); //zapis zakodowanych danych do pliku

    string decoded = huff.readFromFile("output.huff"); //odczyt i dekodowanie
    cout << decoded << endl;

    //zapis zdekodowanego tekstu do pliku tekstowego z polskimi znakami
    ofstream outputText("decoded.txt");
